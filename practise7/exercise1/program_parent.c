...
void signal_handler(int sig) {
    if (sig == SIGUSR1) {
        ...
        printf("Program (parent): Sum is: %d\n", sum);
    }
}
...
void run_child_process() {
    ...
    execlp("./program_child", "program_child", shm_id_str, NULL);
    perror("execlp");
    exit(1);
}
...
void handle_parent_process() {
    while (1) {
        int n, i, input;
        printf("Program (parent): Enter count of numbers to sum (0 - exit): ");
        scanf("%d", &n);
        if (n <= 0) {
            break;
        }
        for (i = 0; i < n; i++) {
            printf("Program (parent): Input num: ");
            scanf("%d", &input);
